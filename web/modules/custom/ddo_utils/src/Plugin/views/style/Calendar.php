...
namespace Drupal\ddo_utils\Plugin\views\style;
 ...
 * @ViewsStyle(
 *   id = "ddo_calendar",
 *   title = @Translation("DDO Calendar"),
 *   help = @Translation("Present view results as a Calendar for domaine d'O."),
...
////// Code personnalisé ///////////////////////////////////////////////////////////////////////////////////////////////
$rendered_fields_keys = [];
foreach($this->rendered_fields as $key=>$element) {
  $element_nid = trim(strip_tags(preg_replace('/<!--(.|\s)*?-->/', '', $element['nid']->__toString())));
  if (isset($rendered_fields_keys[$element_nid])) {
    $rendered_fields_keys[$element_nid][] = $key;
  }
  else {
    $rendered_fields_keys[$element_nid][] = $key;
  }
}
//dpm($rendered_fields_keys);
////// Fin de code personnalisé ////////////////////////////////////////////////////////////////////////////////////////

    foreach ($this->view->result as $row_index => $row) {
//dpm('------------------------------------------------------------  ligne n° ' . $row_index);
//dpm($row->_entity->getTitle());
//$rendered_title = $this->rendered_fields[$row_index]['title']->__toString();
//$rendered_title = strip_tags(preg_replace('/<!--(.|\s)*?-->/', '', $rendered_title));
//dpm($rendered_title);
//dpm($row->node__field_dates_field_dates_value);
...
//dpm(count($events));
      ...
      foreach ($events as $event_index=>$event_info) {

////// Code personnalisé ///////////////////////////////////////////////////////////////////////////////////////////////
$fields_to_render_keys = $rendered_fields_keys[$row->nid];
$key_to_use = $fields_to_render_keys[$event_index];
//dpm($key_to_use);
////// Fin de code personnalisé ////////////////////////////////////////////////////////////////////////////////////////
...
//        $event_info->setRenderedFields($this->rendered_fields[$row_index]);
        $event_info->setRenderedFields($this->rendered_fields[$key_to_use]);
////// Fin de code personnalisé ////////////////////////////////////////////////////////////////////////////////////////
